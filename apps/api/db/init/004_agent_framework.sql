...
CREATE INDEX IF NOT EXISTS ix_agent_projects_user_updated
  ON agent_projects(user_id, updated_at DESC);

CREATE INDEX IF NOT EXISTS ix_create_runs_project_created
  ON create_runs(project_id, created_at DESC);

CREATE INDEX IF NOT EXISTS ix_create_runs_user_created
  ON create_runs(user_id, created_at DESC);

CREATE INDEX IF NOT EXISTS ix_create_run_steps_run_step
  ON create_run_steps(run_id, step_no);

CREATE INDEX IF NOT EXISTS ix_agent_memory_project_type
  ON agent_memory_index(project_id, memory_type, created_at DESC);

CREATE INDEX IF NOT EXISTS ix_agent_memory_tags
  ON agent_memory_index USING gin(tags);
